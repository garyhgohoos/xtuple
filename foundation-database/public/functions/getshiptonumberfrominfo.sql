...
BEGIN
  ...
  WHERE ((UPPER(cust_name)=UPPER(_custname))
    ...
    AND UPPER(addr_city)=UPPER(_city)
    AND UPPER(addr_state)=UPPER(_state)
    AND TRIM(UPPER(addr_postalcode)) =TRIM(UPPER(_postalcode))
  ...
  ELSE
    ...
    IF (_counter = 0) AND (_generate) AND (_create) THEN
      _candidate = UPPER(_fullname);
    ELSE
      SELECT COUNT(*) INTO _counter
      FROM custinfo, shiptoinfo, addr
      WHERE ((UPPER(cust_name)=UPPER(_custname))
...
--
  ...
  IF (_generate) THEN
    ...
    _custnumber, _candidate, _fullname,
